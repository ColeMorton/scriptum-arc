...
describe('Row Level Security (RLS) Logic', () => {
  it('should validate tenant isolation logic', () => {
    ...
    const tenant1Data = {
      tenantId: tenant1Id,
      users: [{ id: 'user-1', email: 'user1@tenant1.com', tenantId: tenant1Id }],
      clients: [{ id: 'client-1', name: 'Tenant 1 Client', tenantId: tenant1Id }],
    }

    const tenant2Data = {
      tenantId: tenant2Id,
      users: [{ id: 'user-2', email: 'user2@tenant2.com', tenantId: tenant2Id }],
      clients: [{ id: 'client-2', name: 'Tenant 2 Client', tenantId: tenant2Id }],
    }
  ...
  it('should validate data filtering by tenant', () => {
    ...
    const filterByTenant = (
      data: Array<{ id: string; name: string; tenantId: string }>,
      tenantId: string
    ) => {
