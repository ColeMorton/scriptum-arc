# Frequently Asked Questions

**Version**: 1.0  
**Last Updated**: 2025-01-27  
**Owner**: Marketing  
**Status**: Client-Facing FAQ

---

## General Questions

### What is n8n?

n8n is an open-source workflow automation platform that connects different systems and automates business processes. Unlike Zapier or Make, n8n is self-hosted, giving you complete control over your automation platform and data.

### Why choose n8n over Zapier or Make?

- **Platform Ownership**: You own your n8n instance and data
- **No Vendor Lock-in**: Export and migrate your workflows anytime
- **Unlimited Scale**: No per-task pricing limits
- **Custom Development**: TypeScript nodes for complex business logic
- **Data Sovereignty**: Your data stays on your infrastructure

### What makes Zixly different from other automation services?

- **Brisbane-Based Expertise**: Local team with Australian business knowledge
- **Platform Ownership Model**: You own your automation platform
- **Flexible Engagement**: Project-based or ongoing support options
- **Complete Stack Services**: n8n plus 20+ self-hostable tools
- **Proven Methodology**: Successful implementations across Brisbane SMEs

---

## Service Questions

### What services do you offer?

- **n8n Platform Setup**: Complete implementation on your infrastructure
- **Custom Workflow Development**: Bespoke workflows for your business processes
- **Integration Services**: Connect your existing systems (Xero, HubSpot, etc.)
- **Training & Support**: Comprehensive training and ongoing assistance
- **Stack Expansion**: Implement additional tools from the self-hostable SME stack

### How much does it cost?

Our services range from $2,500 for basic n8n setup to $30,000+ for comprehensive automation platforms. Most clients invest $5,000-$15,000 for a complete solution. We offer flexible engagement options including project-based fees and monthly retainers.

### How long does implementation take?

- **Basic Setup**: 1-2 weeks
- **Custom Workflows**: 1-4 weeks per workflow
- **Full Implementation**: 4-16 weeks depending on scope
- **Stack Expansion**: 2-6 weeks per additional tool

### Do you offer ongoing support?

Yes, we offer flexible ongoing support options:

- **Monthly Management**: $500-$2,000/month for full platform management
- **On-Demand Consulting**: $150-$250/hour for specific needs
- **Priority Support**: Dedicated support for critical issues

---

## Technical Questions

### Do I need technical expertise?

No, we handle all the technical implementation. We provide comprehensive training so your team can manage the platform, but we're always available for support and new development.

### What systems can you integrate with?

We can integrate with 50+ business systems including:

- **Accounting**: Xero, MYOB, QuickBooks
- **CRM**: HubSpot, Pipedrive, Salesforce
- **Project Management**: Asana, Trello, Monday.com
- **E-commerce**: Shopify, WooCommerce, Magento
- **Communication**: Slack, Microsoft Teams, Email
- **And many more**

### Is my data secure?

Absolutely. Your n8n instance runs on your infrastructure, so you have complete control over your data. We implement security best practices including encryption, secure authentication, and regular backups.

### Can I export my workflows?

Yes, you own your n8n instance and can export all workflows, data, and configurations at any time. There's no vendor lock-in.

---

## Business Questions

### What types of businesses do you work with?

We specialize in Brisbane and South East Queensland SMEs across various industries:

- **Professional Services**: Law firms, accounting practices, consultancies
- **Construction & Trades**: Builders, electricians, plumbers, landscapers
- **E-commerce & Retail**: Online stores, retail businesses
- **Manufacturing**: Small to medium manufacturers
- **And many other industries**

### How do you ensure success?

Our proven methodology includes:

- **Discovery Phase**: Understanding your business processes and needs
- **Custom Development**: Workflows tailored to your specific requirements
- **Comprehensive Training**: Your team learns to manage and expand the platform
- **Ongoing Support**: We're always available for assistance and optimization

### What if I need changes after implementation?

We offer flexible engagement options:

- **Change Requests**: Billed at standard hourly rates
- **New Workflows**: Project-based or hourly development
- **Platform Updates**: Included in monthly management retainers
- **Training**: Additional training sessions as needed

---

## Process Questions

### How do we get started?

1. **Free Consultation**: 30-minute discovery call to understand your needs
2. **Proposal**: Detailed service recommendations and investment breakdown
3. **Agreement**: Sign service agreement to begin implementation
4. **Implementation**: We handle all technical setup and development
5. **Training**: Comprehensive training for your team
6. **Support**: Ongoing assistance as needed

### What information do you need from us?

- **Business Process Overview**: How your current systems work
- **Integration Requirements**: Which systems need to connect
- **Automation Goals**: What processes you want to automate
- **Technical Access**: Temporary access to your systems for integration
- **Team Information**: Who will be using and managing the platform

### Can you work with our existing IT team?

Absolutely. We work collaboratively with your IT team and can provide training to help them manage the platform. We can also provide ongoing support and consultation as needed.

---

## Investment Questions

### What's included in the investment?

Our investments include:

- **Complete Implementation**: Platform setup, workflows, integrations
- **Training**: Comprehensive training for your team
- **Documentation**: Complete documentation and best practices
- **Support**: Initial support period included
- **Knowledge Transfer**: You own and control the platform

### Are there any hidden costs?

No hidden costs. Our proposals include all implementation, training, and initial support. Optional ongoing services are clearly outlined and priced separately.

### Do you offer payment plans?

Yes, we offer flexible payment terms:

- **50% deposit** upon agreement
- **50% upon completion** for project-based work
- **Monthly billing** for ongoing services
- **Quarterly discounts** for annual contracts

### Can I start small and expand later?

Absolutely. Many clients start with basic n8n automation and expand to additional tools and workflows over time. Our flexible engagement model allows you to scale up or down as needed.

---

## Support Questions

### What support do you provide?

- **Implementation Support**: Full setup and configuration
- **Training Support**: Comprehensive team training
- **Ongoing Support**: Platform monitoring and optimization
- **Emergency Support**: Priority assistance for critical issues
- **Consulting Support**: Strategic advice and optimization

### How quickly do you respond to issues?

- **Critical Issues**: 2 hours response time
- **High Priority**: 4 hours response time
- **Standard Issues**: 24 hours response time
- **Support Hours**: 9 AM - 5 PM AEST (extended for critical issues)

### Do you provide training?

Yes, we provide comprehensive training including:

- **User Training**: How to use and manage workflows
- **Administrator Training**: Platform management and configuration
- **Best Practices**: Optimization and maintenance
- **Documentation**: Complete guides and procedures

---

## Next Steps

### How do I get started?

1. **Book a Free Consultation**: Schedule a 30-minute discovery call
2. **Receive a Proposal**: Detailed service recommendations
3. **Start Implementation**: Begin your automation journey
4. **Scale & Optimize**: Expand and improve over time

### What should I prepare for the consultation?

- **Business Process Overview**: Current workflows and pain points
- **System Information**: What systems you currently use
- **Automation Goals**: What you want to achieve
- **Questions**: Any specific questions about n8n or our services

### How do I contact you?

- **Email**: [email address]
- **Phone**: [phone number]
- **Website**: [website URL]
- **LinkedIn**: [LinkedIn profile]

---

**Ready to automate your business with n8n?**

[Book Your Free Consultation](#) | [Download Service Guide](#) | [View Case Studies](#)

---

**Document Version**: 1.0  
**Last Updated**: 2025-01-27  
**Owner**: Zixly Service Delivery  
**Review Cycle**: Quarterly
