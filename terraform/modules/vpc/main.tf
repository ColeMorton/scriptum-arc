...
resource "aws_subnet" "elasticache" {
  count             = length(var.availability_zones)
  vpc_id            = aws_vpc.main.id
  cidr_block        = cidrsubnet(var.vpc_cidr, 4, count.index + 30)
  availability_zone = var.availability_zones[count.index]

  tags = merge(
    var.common_tags,
    {
      Name = "${var.environment}-elasticache-${substr(var.availability_zones[count.index], -1, 1)}"
      Type = "elasticache"
    }
  )
}
...
resource "aws_nat_gateway" "main" {
  ...
  tags = merge(
    var.common_tags,
    {
      Name = "${var.environment}-nat-${substr(var.availability_zones[count.index], -1, 1)}"
    }
  )
